...
def get_person(request, person_name, year):
    ...
    for ii in range(12):
        monthly_data = {
            "name": month_names[ii],
            "name_short": month_names_short[ii],
            "data": [],
        }
...
def set_mood(request, person_name, year, month, day, mood):
    ...
    old = MoodOfTheDay.objects.filter(
        person_name=person_name, year=year, month=month, day=day
    )
    if len(old) > 0:
        old.delete()

    m = MoodOfTheDay(
        person_name=person_name, year=year, month=month, day=day, mood=mood
    )
    m.save()
